...
const aesDecrypt = async (data: string, password: string): Promise<string> => {
  if (!window.crypto?.subtle) throw new Error("Web Crypto API not available");
...
const xorDecrypt = (data: string, password: string): string => {
  let text = atob(data);
  let out = "";
  for (let i = 0; i < text.length; i++) {
    out += String.fromCharCode(text.charCodeAt(i) ^ password.charCodeAt(i % password.length));
...
const EncryptionDecryptionClient: React.FC = () => {
...
export default EncryptionDecryptionClient;
