...
// Minimal MD4 implementation (same as used for NTLM)
function md4(input: string): string {
  function toUtf8(str: string) {
    return new Uint8Array(unescape(encodeURIComponent(str)).split('').map(c => c.charCodeAt(0)));
  }
  function lrot(n: number, b: number) { return ((n << b) | (n >>> (32 - b))) >>> 0; }
  function md4buf(buf: Uint8Array): string {
  ...
  return md4buf(toUtf8(input)).toUpperCase();
}

export default function Md4HashGenerator() {
  ...
  function handleGenerate() {
    ...
    try {
      ...
      setError("Error generating MD4 hash.");
      setOutput("");
    }
  }
